    ...
    while True:
        ...
        if count % sample_count == 0:  # Every 50 readings
            ...
            if (np.round(average_change, 2) == 0) and (mode_value == 0) and (np.round((np.sum(np.unique(voltageChangeArr_np))), 0) == 0):
                print("NO WAVE")
            if (len(np.unique(voltageChangeArr_np)) >= 2) and (average_change > 0) and np.any(voltageChangeArr_np > 1) and (np.round(mode_value, 2) == 0 or ((np.max(voltageChangeArr)) - mode_value <= 0.12)) :
                if (mode_count > 10):
                    print("SQUARE WAVE")
