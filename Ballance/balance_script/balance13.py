...
def setup_pid():
    ...
    pid_x = PID(0.05, 0.01, 0.08, setpoint=2000)  # Kp, Ki, Kd for X axis
    pid_y = PID(0.05, 0.01, 0.08, setpoint=2000)  # Kp, Ki, Kd for Y axis

    pid_x.output_limits = (-5, 10)  # Step limits for motor 2 (X axis)
    pid_y.output_limits = (-5, 10)  # Step limits for motor 1 (Y axis)
