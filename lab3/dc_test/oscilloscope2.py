    ...
    while True:
        ...
        if count % sample_count == 0:  # Every 50 readings
            ...
            if (average_change > 0 and mode_count > 8) and (len(np.unique(voltageChangeArr_np)) > 7) and (mode_value != 0):
                if((np.round(freq, 2) <= 2) and ((np.round(freq, 2) >= 1) and mode_count > 20)):
                    print("TRIANGLE WAVE")
            ...
            if (len(np.unique(voltageChangeArr_np)) > 4) and average_change > 0 and mode_count <= 9:
                ...
                if (mode_value != 0):
                    print("SINE WAVE")
            elif ((np.round(freq, 2) <= 2) and ((np.round(freq, 2) >= 1) and mode_count > 9 and len(np.unique(voltageChangeArr_np)) > 4 and average_change > 0)):
                print("SINE WAVE")
...
def main():
    measure_voltage(sample_rate=75)  # 75 samples per second
